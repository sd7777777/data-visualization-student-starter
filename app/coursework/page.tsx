...
const assignments = [
  { week: 'WEEK 01', title: 'Repository Setup', status: 'Fork-ready', items: ['The live explorer and D3 charts now reside under the required src/assignments/week-01 path.', 'The assignment registry follows the starter repository’s src/assignments/index.ts convention.', 'The local Git history descends from the course starter, and the Pages workflow handles a repository-specific base path.', 'Creating the visible GitHub fork and supplying its URLs remain account-specific steps.'] },
  ...
  return <main>
    ...
    <section className="section-wrap">
      ...
      <div className="task-grid">
        <article className="task-card"><span className="eyebrow">01 / FORK</span><h3>Fork Curran’s starter on GitHub first.</h3><p>This server-side action creates the “forked from” relationship the assignment expects. Publishing a brand-new repository is not equivalent.</p></article>
        <article className="task-card"><span className="eyebrow">02 / LOCAL REPOSITORY</span><h3>Connect this prepared branch to your fork.</h3><p>The branch already includes the starter’s history and required assignment paths. Once your fork exists, its URL can become the repository’s primary remote.</p></article>
        <article className="task-card"><span className="eyebrow">03 / LIVE PAGE</span><h3>Enable GitHub Actions for Pages.</h3><p>Choose GitHub Actions as the Pages source. The included workflow builds the same three-route static site and adjusts asset paths for the fork’s repository name.</p></article>
        <article className="task-card"><span className="eyebrow">04 / SUBMIT</span><h3>Share one screenshot, both links, and a short description.</h3><p>Describe the project as an interactive analysis of 1.4 million 2024 Medicare Part D provider records using Python preprocessing and D3.</p></article>
      </div>
    </section>
    <section className="section-wrap">
